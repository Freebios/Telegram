...
public class OverlaysView extends View implements ProfileGalleryView.Callback {
    ...
    public OverlaysView(ProfileActivity profileActivity, Context context) {
        super(context);

        this.profileActivity = profileActivity;
        setVisibility(GONE);

        statusBarHeight = profileActivity.getActionBar().getOccupyStatusBar() && !profileActivity.getInBubbleMode() ? AndroidUtilities.statusBarHeight : 0;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int count = profileActivity.getAvatarsViewPager().getRealCount();
        selectedPosition = profileActivity.getAvatarsViewPager().getRealPosition();
        ...
        if (count > 1 && count <= 20) {
            ...
            int y = AndroidUtilities.dp(4) + (Build.VERSION.SDK_INT >= 21 && !profileActivity.getInBubbleMode() ? AndroidUtilities.statusBarHeight : 0);
            for (int a = 0; a < count; a++) {
                ...
                if (a == previousSelectedPotision && Math.abs(previousSelectedProgress - 1.0f) > 0.0001f) {
                    ...
                    if (profileActivity.getAvatarsViewPager().isCurrentItemVideo()) {
                        progress = currentProgress = profileActivity.getAvatarsViewPager().getCurrentItemProgress();
                        if (progress <= 0 && profileActivity.getAvatarsViewPager().isLoadingCurrentVideo() || currentLoadingAnimationProgress > 0.0f) {
